...
#define PINCOL (((PINC &= 0x30) >> 4) | ((PINB &= _BV(4)) >> 2))
...
void Timer0Init()
{
	TCCR0B = (_BV(CS01) | _BV(CS00));	 //Preskaler CLKIO/64
...
void itoLED (uint16_t liczba)
{
	for(uint8_t i = LEDDISPNO; i > 0; i--)
	{
		LEDDIGITS[i-1] = liczba%10;
		liczba /= 10;
	}
}
